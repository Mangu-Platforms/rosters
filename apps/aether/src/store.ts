...
export interface Emp {
  id: string; first: string; last: string; role: string; rate: number; site: string;
  office?: boolean; hue: number; entity: string; ref: string; status: "active" | "terminated";
}
export interface Ex {
...
export const CRAFT: Record<string, number> = {
  Carpenter: 48.53, Laborer: 38.2, "Apprentice Laborer": 26.74,
  Electrician: 58.9, "Equipment Operator": 52.1, Foreman: 53.38,
};
...
export function detect(emps: Emp[], wd: boolean, entity: string): Ex[] {
...
export const useA = create<S>()((set, get) => ({
  ...boot(),
